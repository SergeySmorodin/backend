...
class DatabasePopulator:
    ...
    def create_superuser(self, username='admin', password='admin', email='admin@example.com'):
    ...
    def print_summary(self):
        ...
        if self.users:
            for user in self.users:
                ...
                shared_count = UserFile.objects.filter(user=user, share_token__isnull=False).count()
                print(f"  • {user.username}: {file_count} файлов ({shared_count} расшарено)")
